...
export default function CourseDetail() {
  const { selectedCourse, selectedCourseId, setSelectedLesson } = useApp();
  ...
  function statusIcon(status) {
    if (status === "completed")
      return <CheckCircleIcon size={16} className="text-white" />;
    if (status === "current")
      return <PlayIcon size={10} className="text-white" />;
    return <LockIcon size={13} className="text-[var(--text-subtle)]" />;
  }
