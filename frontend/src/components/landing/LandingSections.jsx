...
const STEPS = [
  { title: "Pilih & Kenalan", text: "Pilih topik yang kamu suka, lalu kenali setiap gerakan BISINDO.", icon: BookIcon, tone: "blue" },
  { title: "Main & Berlatih", text: "Tirukan gerakannya lewat aktivitas seru yang gampang diikuti.", icon: PlayIcon, tone: "yellow" },
  { title: "Rayakan Progresmu", text: "Lihat kemajuanmu dan rayakan setiap langkah yang berhasil kamu lewati.", icon: ChartIcon, tone: "mint" },
];

const BENEFITS = [
  { title: "Belajar Sambil Bermain", text: "Kenalan dengan BISINDO lewat aktivitas dan permainan yang terasa menyenangkan.", icon: PlayIcon, tone: "blue", label: "Seru" },
    ...
    <section id="beranda" className="kids-hero" aria-labelledby="kids-hero-title">
      ...
      <div className="kids-container kids-hero-grid">
        <div className="kids-hero-copy">
          <motion.p className="kids-hero-eyebrow" {...entrance(0.1, 16)}><span aria-hidden="true">✨</span> Belajar BISINDO jadi makin seru!</motion.p>
          <h1 id="kids-hero-title" className="kids-hero-title">
            <motion.span className="kids-hero-line" {...entrance(0.18)}>Belajar Bahasa Isyarat</motion.span>
            <motion.span className="kids-hero-line kids-hero-line-accent" {...entrance(0.28)}><span className="kids-hero-phrase">Jadi Lebih Seru</span>{" "}<span className="kids-hero-phrase">Bersama!</span></motion.span>
          </h1>
          <motion.p className="kids-hero-subtitle" {...entrance(0.43)}>
            Yuk kenalan dengan BISINDO lewat permainan, gerakan, dan aktivitas seru yang mudah diikuti!
          </motion.p>
        ...
        >
          <div className="kids-hero-float">
            <div ref={illustrationMotion.ref} className="kids-hero-image-shell" {...illustrationMotion.pointerProps}>
              ...
              >
                Ekspresi wajah <span className="kids-gesture-tooltip">Ekspresi wajah membantu menyampaikan makna.</span>
              </button>
              <button
                type="button"
          ...
          <p className="kids-hero-visual-caption"><span className="kids-status-check"><CheckCircleIcon size={18} /></span> Gerakan dan ekspresinya terlihat jelas</p>
        </motion.div>
        <div className="kids-quick-actions" aria-label="Akses cepat">
          <QuickActionCard href={APP_DESTINATIONS.howToLearn.href} tone="game" title="Cara Belajar" description="Cuma 3 langkah mudah" icon={PlayIcon} delay={0.62} reducedMotion={reducedMotion} />
          <QuickActionCard href={APP_DESTINATIONS.signDemo.href} tone="progress" title="Coba Gerakan" description="Yuk praktik BISINDO!" icon={HandSignIcon} delay={0.7} reducedMotion={reducedMotion} />
        </div>
      </div>
      <a className={`kids-scroll-cue ${scrollCueVisible ? "" : "is-hidden"}`} href={APP_DESTINATIONS.topics.href} aria-label="Lanjut ke pilihan topik belajar">
        <span>Yuk, Lanjut!</span><ChevronDownIcon size={18} />
      </a>
    </section>
  );
}
...
function TopicCard({ topic, onStart }) {
  ...
  const actionLabel = topic.status === "Selesai" ? "Ulangi lagi" : topic.status === "Sedang dipelajari" ? "Yuk lanjut!" : "Yuk mulai!";
  const statusLabel = {
    "Sedang dipelajari": "Lagi dipelajari",
    "Belum dimulai": "Siap dicoba",
    Selesai: "Sudah selesai",
    Terkunci: "Masih terkunci",
  }[topic.status];
  ...
  return (
    <article ref={pointerMotion.ref} className={`kids-topic-card tone-${topic.tone} ${locked ? "is-locked" : ""}`} {...pointerMotion.pointerProps}>
      ...
      <span className="kids-topic-status">{locked && <LockIcon size={13} />} {statusLabel}</span>
      ...
      {locked ? <span className="kids-topic-locked-note">Selesaikan topik sebelumnya, ya</span> : <button type="button" className="kids-card-action" onClick={onStart} aria-label={`Mulai belajar topik ${topic.title}`}>{actionLabel} <ArrowRightIcon size={16} /></button>}
    </article>
  );
}

export function TopicSection({ onStart }) {
  return <section id="topik" className="kids-section kids-topics-section" aria-labelledby="topic-title"><div className="kids-topic-decor" aria-hidden="true"><i /><i /><i /></div><div className="kids-container"><Reveal><Heading eyebrow="Petualangan belajarmu" id="topic-title" title="Mau Belajar Apa Hari Ini?" text="Pilih topik yang paling bikin kamu penasaran. Setiap pelajaran singkat, visual, dan gampang diikuti." /></Reveal><div className="kids-topic-grid">{TOPICS.map((topic, index) => <Reveal key={topic.title} delay={index * 70}><TopicCard topic={topic} onStart={onStart} /></Reveal>)}</div></div></section>;
}

export function LearningJourneySection() {
  const reducedMotion = useReducedMotion();
  const { ref, inView } = useInView({ threshold: 0.18 });
  return <section ref={ref} id="cara-belajar" className="kids-section kids-journey-section" aria-labelledby="journey-title"><div className="kids-container"><Reveal><Heading eyebrow="Cuma tiga langkah" id="journey-title" title="Gimana Cara Belajarnya?" text="Pilih topik, ikuti gerakannya, lalu rayakan progresmu bersama SignLearn Kids." /></Reveal><ol className={`kids-journey ${inView ? "is-visible" : ""}`}>{STEPS.map((step, index) => { const Icon = step.icon; return <motion.li key={step.title} className={`tone-${step.tone}`} initial={reducedMotion ? false : { opacity: 0.42, y: 24, scale: 0.97 }} animate={inView ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0.42, y: 24, scale: 0.97 }} transition={{ duration: reducedMotion ? 0 : 0.5, delay: reducedMotion ? 0 : index * 0.22, ease: premiumEase }}><span className="kids-journey-number">{String(index + 1).padStart(2, "0")}</span><span className="kids-journey-icon" aria-hidden="true"><Icon size={31} /></span><h3>{step.title}</h3><p>{step.text}</p><span className="kids-step-personality" aria-hidden="true">{index === 0 ? <><i /><i /><i /></> : index === 1 ? <PlayIcon size={14} /> : <><i /><i /><i /></>}</span></motion.li>; })}</ol></div></section>;
}
...
export function BenefitsSection() {
  return <section id="manfaat" className="kids-section kids-benefits-section" aria-labelledby="benefits-title"><div className="kids-container"><Reveal><Heading eyebrow="Belajar dengan nyaman" id="benefits-title" title="Kenapa Seru Belajar di Sini?" text="Karena belajar BISINDO bisa terasa seru, aman, dan gampang dipahami." /></Reveal><div className="kids-benefits-grid">{BENEFITS.map((item, index) => <Reveal key={item.title} delay={index * 80}><BenefitCard item={item} /></Reveal>)}</div></div></section>;
}

export function ProgressPreviewSection({ onProgress }) {
  return <section id="progres" className="kids-section kids-progress-section" aria-labelledby="progress-title"><div className="kids-container"><div className="kids-progress-card"><Reveal><div><p className="kids-eyebrow">Intip progresmu</p><h2 id="progress-title">Setiap Langkah Pantas Dirayakan</h2><p className="mt-3 text-[#5C6B76]">Setelah mulai belajar, kamu bisa melihat kebiasaan, pelajaran, dan penghargaanmu di satu tempat.</p><div className="kids-progress-stats"><div><FireIcon /><strong>Hari belajar</strong><span>jaga kebiasaanmu</span></div><div><CheckCircleIcon /><strong>Pelajaran</strong><span>lihat yang selesai</span></div><div><StarIcon filled /><strong>Bintang</strong><span>rayakan usahamu</span></div></div><p className="kids-level"><TrophyIcon /> Semua pencapaian tersimpan di <strong>akun belajarmu</strong></p><button type="button" className="kids-button kids-button-primary mt-6" onClick={onProgress}>Masuk & Lihat Progres</button></div></Reveal><div className="kids-mascot-celebrate" aria-label="Finn menyemangati perjalanan belajar"><StarIcon size={44} filled /><strong>Hebat!</strong><span>Setiap usaha itu berarti</span></div></div></div></section>;
}
