...
export default function Landing() {
  ...
  return <div className="kids-page min-h-screen"><a className="skip-link" href="#main-content">Lewati ke konten utama</a><LandingHeader onLogin={() => navigate("/login")} onRegister={() => navigate("/register")} onAccessibility={() => setAccessibilityOpen(true)} /><main id="main-content"><KidsHero onStart={() => navigate("/register")} onTrySign={() => scrollTo("demo-gerakan")} /><SignDemoSection /><TopicSection onStart={() => navigate("/register")} /><LearningJourneySection /><BenefitsSection /><ProgressPreviewSection onProgress={() => navigate("/login")} /><ParentTrustSection onGuide={() => scrollTo("footer-support")} /><FinalKidsCTA onStart={() => navigate("/register")} onExplore={() => scrollTo("topik")} /></main><LandingFooter /><AccessibilityMenu open={accessibilityOpen} onClose={() => setAccessibilityOpen(false)} /></div>;
}
